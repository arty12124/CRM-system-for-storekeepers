...
CONFIG = {
    ...
    "default_image": "assets/default_image.png"
}
...
MAX_LOG_LINES = 100  # Максимальное количество строк в логе

# Конфигурация для парсинга Avito
DEBUG_PORT = 9222
AVITO_ORDERS_URL = "https://www.avito.ru/orders?source=sidebar"
CHROME_START_TIMEOUT = 10
COOKIES_FILE = "database/avito_cookies.json"
...
class MarketplaceCRM(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(main_frame, text="Заказы с маркетплейсов", font=ctk.CTkFont(size=24, weight="bold")).grid(row=0, column=0, pady=(0, 20), sticky="w")
        ...
        self.table.bind("<Double-1>", self.change_status)
    ...
    def change_status(self, event):
        ...
        item = self.table.identify_row(event.y)
        if not item or self.table.identify_column(event.x) != "#7":
            return
        external_id = self.table.item(item, "tags")[0]
    ...
    def process_wildberries_order(self, order, existing_orders, conn, cursor):
        ...
        try:
            ...
            product = products[0]
            cursor.execute("""
                INSERT INTO orders (external_id, product_image, product_name, product_link, quantity, price, source, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                external_id,
    ...
    def process_avito_order(self, order, existing_orders, conn, cursor):
        ...
        try:
            ...
            status_map = {
                ...
                "Подтверждён": "Новый",
                "Отправьте заказ": "Новый",  # Добавлен новый статус
